...
model_levels <- c("rw", "lrw", "R_random3", "R_biased3", "expdelay", "incdelay3", "powdelay", "sigmoid", "gompertz")
place_levels <- c("BCN", "LON", "MAD", "NYC", "SF", "HOMO", "HET", "SIM")
epi_phase_levels <- c(1:20, "TOTAL")
id_cols <- c("model","cutoff","epi_phase","place", "sr")
full_names <- c(rw="Random walk", lrw="Log random walk", R_random="R random walk", R_biased="R biased random", expdelay="Exponential time", incdelay3="Exponential attack rate", powdelay="Power law attack rate", sigmoid="Sigmoid")

...
data <- read.table("data/eul_natsal_2010_for_archive.tab", header = TRUE,    # does the file have column headers?
...
model_type_dict <- c(rw = "statistical", lrw = "statistical", AR="statistical", biaslrw = "statistical", R_biased = "semi-mechanistic", lin = "mechanistic", exp = "mechanistic", inc = "mechanistic", 
                     lindelay = "mechanistic", expdelay = "mechanistic", expdelay2 = "mechanistic", incdelay = "mechanistic", inc2delay = "mechanistic", comp = "mechanistic", phenom = "phenom", sigmoid_inf="sigmoid_inf")

rw <- stan_model("functions/random_walk.stan")
lrw <- stan_model("functions/log_random_walk.stan")
R_random5 <- stan_model("functions/R_random5.stan")
expdelay <- stan_model("functions/expdelay.stan")
incdelay3 <- stan_model("functions/incdelay3.stan")
powdelay <- stan_model("functions/powdelay.stan")
sigmoid <- stan_model("functions/sigmoid.stan")

rwP <- stan_model("functions/random_walkP.stan")
...
model_list <- list(#rw=rw, lrw=lrw, expdelay=expdelay, incdelay3=incdelay3, powdelay=powdelay, R_random5=R_random5, sigmoid=sigmoid,
                   rwP=rwP, lrwP=lrwP, expdelayP=expdelayP, incdelayP=incdelayP, powdelayP=powdelayP, R_random5=R_random5, sigmoidP=sigmoidP)

country_data <- read.csv("data/mpox_cases2.csv")

#country_linelist <- read.csv("data/mpox_cases.csv")
#cat <- country_linelist %>% group_by(Country, Date_confirmation) %>% summarise(count=n()) %>% filter(Country=="France") %>% mutate(date=as.Date(Date_confirmation))
#ggplot(cat, aes(x=date, y=count)) + geom_point() + theme_bw()
...
ggplot(dat[1:200,] %>% tidyr::gather("city", "value", 2:6), aes(x=date, y=value)) +
  geom_line(aes(color=city)) +
  theme_bw()


#rstan::expose_stan_functions(model_strat2)
...
fit_func <- function(simulation, model_name, range_limits, strat, cutoff, tag, keep_fit=F, epi_phase=NA, decay_type=NA, dir="", iter=1000, chains=4, adapt_delta=0.8){
  ...
  p <- proc.time()
  fit <- sampling(model, input_list, iter=iter, chains=chains, cores=8, seed=(3), init=init, control = list(adapt_delta = adapt_delta)) 
...
for(j in c(list.files("simulation/sim_2/"))[1:100]){
  ...
  for(i in times){
    for(p in c("lrwP", "R_random5", "expdelayP", "incdelayP", "powdelayP", "sigmoidP")){
...
for(j in c(list.files("simulation/sim_homo/")[1:30])){
...
for(j in c(list.files("simulation/sim_het/")[1:30])){
  print(j)
  ...
  for(i in times){
    for(p in c("R_random5")){
      a5 <- fit_func(simulation, model_name=p, range_limits=c(0), strat="annual", cutoff=i, tag=tag, keep_fit=TRUE, epi_phase=which(times==i), decay_type=NA, dir="fit_hetero")
...
#for(i in 1:20) print(rstan::extract(readRDS(paste0("fits/sim/fit_incdelay_k=1_strat=annual_likelihood=1_epi_phase=10_",i,".rds"))$fit)$delta %>% mean())
...
plotting_func <- function(fit_output){
  ...
  if(fit_output$model_name %in% c("sat_lint", "sat_expt", "sat_inc", "sat_mix")){
    inf <- array(inf, dim=c(dim(inf), 1))
    input$incidence_strat <- as.matrix(input$incidence_strat, ncol=1)
  } 
  ...
  #inc <- rbind(fit_output$simulation$incidence$incidence[1:390])

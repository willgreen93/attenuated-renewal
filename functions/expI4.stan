...
parameters {
  ...
  vector[k] R_floor_frac;              // Initial reproduction number per group
...
transformed parameters {
  ...
  for (j in 1:k) {
    R_floor[j] = R0[j]*(1/(1+exp(-R_floor_frac[j])));
    ...
    for (t in 2:(N + h)) {
      ...
      Rt = R_floor[j] + (R0[j] - R_floor[j]) * exp(-delta[j] * prop_infected);

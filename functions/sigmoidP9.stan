...
parameters {
  ...
  real<lower=0> t_change;     // Logistic midpoint t_mid (repurposed)
  real<lower=0> phi;                   // Overdispersion
  vector<lower=0>[k] seed_infections;  // Initial infections
  vector<lower=0, upper=1>[k] R_floor;
}
  ...
  for (j in 1:k) {
    ...
    for (t in 2:(N + h)) {
      // Sigmoid (logistic) fall-off for Rt:
        ...
        real susc_frac = (1 - cumulative_infections[t - 1, j] / S0[j]);
        susc_frac = fmax(susc_frac, 0);
        
        infections[t, j] = fmax(1e-6, Rt * infectivity * susc_frac);
...
model {
  ...
  R_floor ~ normal(0.5, 0.25);
  //delta    ~ normal(0.05, 0.0125);     // steepness (per day); larger = sharper drop
  delta ~ lognormal(-2.5, 0.5);
  t_change ~ normal(N/2, N/4);          // midpoint within observed window
